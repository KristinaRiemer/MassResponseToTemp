...
species_stats = read.csv("results_expand_thresholds/species_stats.csv")
individuals_data = read.csv("results_expand_thresholds/stats_data.csv")
...
species_stats = species_stats[species_stats$year_range >= 40 & species_stats$lat_range >= 10,]
...
all_species = species_scatterplot(species_list[1])
plot_fig1 = ggdraw() +
  draw_plot(plot_locations, 0, 0.25, 1, 0.75) +
  draw_plot(all_species, 0, 0, 0.33, 0.3)
...
plot_stats = ggplot(species_stats, aes(temp_r, fill = temp_stat_sig)) +
  ...
  coord_cartesian(xlim = c(-1, 1), ylim = c(0, 100)) +
  labs(x = "r", y = "Number of species", fill = "Statistical significance: ") +
  geom_vline(xintercept = 0, size = 1) +
  theme(legend.position = "top", 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  annotate("text", x = c(-0.75, -0.15, 0.6), y = c(12, 129, 9), label = c("18%", "73%", "9%"))
...
ggsave("results_expand_thresholds/fig_expand_thresholds.jpg", width = 6, height = 12)
